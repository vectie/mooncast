...
1. Complete the governed production/editor lifecycle and create a successful
   3–8 minute export.
2. Promote and review the exact master through Mooncast's production gates.
3. Create `mooncast.final-deliverable-evidence.v1`, binding the project,
   episode, export job, master/render digests, lineage references, duration, and
   explicit no-publication/no-external-effect flags.
4. After governed delivery, record any measurable result as
...
5. Prepare `mooncast.external-handoff-request.v1`. Its transport contract is
...
6. An authorized host transfers the request to MoonFlow. That external effect
   is outside this pack and requires its own authority.
7. MoonFlow reads the bundle through
